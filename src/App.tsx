...
export default function App() {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (fbUser) => {
      if (fbUser) {
        ...
        if (userDoc.exists()) {
          ...
          if (isOwner) await setDoc(doc(db, 'admins', fbUser.uid), { email: OWNER_EMAIL });
        }
      } else {
        setCurrentUser(null);
      }
  ...
  useEffect(() => {
    ...
    const unsubApps = onSnapshot(qApps, (s) => setApps(s.docs.map(d => ({ ...d.data(), id: d.id } as AppEntry))));
    return () => unsubApps();
  }, []);
  ...
  useEffect(() => {
    if (currentUser) {
      ...
      const unsubUsers = onSnapshot(qUsers, (s) => setUsers(s.docs.map(d => ({ ...d.data(), id: d.id } as UserType))));
      return () => unsubUsers();
    }
  }, [currentUser]);
  ...
  useEffect(() => {
    if (currentUser) {
      ...
      // Note: Firestore onSnapshot will enforce security rules locally too
      const unsub = onSnapshot(qMsgs, (s) => setMessages(s.docs.map(d => ({ ...d.data(), id: d.id } as MessageType))));
...
const handlePhoneSignIn = async (e: FormEvent) => {
  ...
  const handleUpload = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const apkFile = (e.currentTarget.querySelector('input[type="file"]') as HTMLInputElement)?.files?.[0];
